...
# 如果没有文件夹就创建一个文件夹
sample_dir = os.path.join(os.getcwd(), "vae_samples")
if not os.path.exists(sample_dir):
    os.makedirs(sample_dir)
else:
    [os.remove(os.path.join(sample_dir, filename)) for filename in os.listdir(
        sample_dir) if "sampled" in filename or "reconst" in filename]
...
dataset = CustomImageDataset(
    annotations_file="C:/Users/chenyuzhuo/Desktop/ML/scripts/torch_test/data_2/id_prop_SV_DV.csv",
...
class VAE(nn.Module):
    def __init__(self):
        ...
        self.convtrans1 = nn.ConvTranspose2d(
            h3_dim, h2_dim, kernel_size=5, stride=1, padding=0)  # [128,16,5,5]
        self.pooltrans1 = nn.MaxUnpool2d(
            kernel_size=2, stride=2)   # [128,16,10,10]

        self.convtrans2 = nn.ConvTranspose2d(
            h2_dim, h1_dim, kernel_size=5, stride=1, padding=0)  # [128,6,14,14]
        self.pooltrans2 = nn.MaxUnpool2d(
            kernel_size=2, stride=2)   # [128,6,28,28]
...
for epoch in range(num_epochs):
    ...
    for i, (x, y) in enumerate(data_loader):
        ...
        reconst_loss = F.mse_loss(
            x_reconst, x.view(-1, num_channels, 32, 32), size_average=False)
        kl_div = - kl_weight * \
    ...
    with torch.no_grad():
        ...
        out = model.decode(z, indices_1, indices_2).view(-1,
                                                         num_channels, 32, 32)
        out = out[:, [1], :, :]   # 只读取某一通道
        save_image(
            out,
        ...
        x_concat = torch.cat(
            [x.view(-1, 1, 32, 32), out.view(-1, 1, 32, 32)], dim=3)
        save_image(
            x_concat,
            os.path.join(
                sample_dir,
